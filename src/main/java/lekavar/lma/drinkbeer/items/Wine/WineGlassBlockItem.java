...
public class WineGlassBlockItem extends BlockItem implements ICapabilityProvider {
    protected final static float MAX_PLACE_DISTANCE = (float) 2;

    public WineGlassBlockItem(Block block, Properties properties) {
    ...
    public void giveEmptyGlassBack(LivingEntity user) {
        if (!(user instanceof Player) || !((Player) user).isCreative()) {
            ItemStack emptyGlassItemStack = new ItemStack(ItemRegistry.EMPTY_WINE_GLASS.get(), 1);
