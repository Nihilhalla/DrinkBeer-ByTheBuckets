...
public class ClientEvents {
  ...
  private static void renderOffhandAttackIndicator(RenderGuiOverlayEvent.Post event) {
    ...
    if (overlay != ForgeGui.CROSSHAIR_ELEMENT && overlay != ForgeGui.HOTBAR_ELEMENT) {
      return;
    }
    ...
    switch (settings.attackIndicator) {
      case CROSSHAIR:
        if (overlay == ForgeGui.CROSSHAIR_ELEMENT && minecraft.options.getCameraType().isFirstPerson()) {
        ...
        break;
      case HOTBAR:
        if (overlay == ForgeGui.HOTBAR_ELEMENT && minecraft.cameraEntity == minecraft.player) {
