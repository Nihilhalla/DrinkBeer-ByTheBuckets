package com.nihilhalla.drinkbeer.utilsborrowedfromMdiyo.client.model.util;
...
public class SimpleBlockModel implements IModelGeometry<SimpleBlockModel> {
  ...
  static final ResourceLocation BAKE_LOCATION = DrinkBeer.getResource("dynamic_model_baking");
  ...
  private static BlockModel getParent(Function<ResourceLocation,UnbakedModel> modelGetter, Set<UnbakedModel> chain, ResourceLocation location, String name) {
    ...
    if (unbaked == null) {
      DrinkBeer.LOG.warn("No parent '{}' while loading model '{}'", location, name);
      return null;
    }
    // no loops in chain
    if (chain.contains(unbaked)) {
      DrinkBeer.LOG.warn("Found 'parent' loop while loading model '{}' in chain: {} -> {}", name, chain.stream().map(Object::toString).collect(Collectors.joining(" -> ")), location);
      return null;
    }
