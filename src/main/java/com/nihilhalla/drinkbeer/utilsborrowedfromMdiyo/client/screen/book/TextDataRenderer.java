...
public class TextDataRenderer {
    ...
    for (TextData item : data) {
      ...
      if (item.useOldColor) {
        ...
        if(colFormat != null) {
          ...
          DrinkBeer.LOG.error("Failed to parse color: " + item.color + " for text rendering.");
        }
      }
  ...
  public static String translateString(String s) {
    ...
    if (s.indexOf("$(") > s.indexOf(")$") || s.contains(")$")) {
      DrinkBeer.LOG.error("[Books] [TextDataRenderer] Detected unbalanced localization symbols \"$(\" and \")$\" in string: \"" + s + "\".");
    }
