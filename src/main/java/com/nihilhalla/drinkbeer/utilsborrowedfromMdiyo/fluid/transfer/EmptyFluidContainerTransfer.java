...
public class EmptyFluidContainerTransfer implements IFluidContainerTransfer {
  public static final ResourceLocation ID = DrinkBeer.getResource("empty_item");
    ...
    if (simulated == this.fluid.getAmount()) {
      ...
      if (actual > 0) {
        if (actual != this.fluid.getAmount()) {
          DrinkBeer.LOG.error("Wrong amount filled from {}, expected {}, filled {}", stack.getItem().getRegistryName(), this.fluid.getAmount(), actual);
        }
