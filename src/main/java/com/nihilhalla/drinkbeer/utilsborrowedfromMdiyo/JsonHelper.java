...
public class JsonHelper {
  ...
  public static JsonObject getJson(Resource resource) {
    try (BufferedReader reader = new BufferedReader(new InputStreamReader(resource.getInputStream(), StandardCharsets.UTF_8))) {
      ...
      DrinkBeer.LOG.error("Failed to load JSON from resource " + resource.getLocation(), e);
      return null;
    }
  }
    ...
    return manager
      ...
      .flatMap(namespace -> {
        ...
        try {
          ...
          DrinkBeer.LOG.error("Failed to load JSON files from {}", location, e);
        }
        return Stream.empty();
      })
      .map(preferredPath != null ? resource -> {
        ResourceLocation loaded = resource.getLocation();
        DrinkBeer.LOG.warn("Using deprecated path {} in pack {} - use {}:{} instead", loaded, resource.getSourceName(), loaded.getNamespace(), preferredPath);
